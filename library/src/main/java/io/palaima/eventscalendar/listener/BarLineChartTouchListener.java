...
package io.palaima.eventscalendar.listener;
...
public class BarLineChartTouchListener extends ChartTouchListener {
    ...
    private final ResourcesHolder resources;
    /**
    ...
    public BarLineChartTouchListener(CalendarView chart, ResourcesHolder resources, Matrix touchMatrix) {
        ...
        transformer = chart.getTransformer();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (!mChart.getProperties().isDragEnabled()
            && !mChart.getProperties().isScaleXEnabled()
            && !mChart.getProperties().isScaleYEnabled()
            && !mChart.getProperties().isPinchZoomEnabled())
            return false;
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (event.getPointerCount() >= 2) {
                    ...
                    if (mSavedDist > 10f) {
                        ...
                        if (mChart.getProperties().isPinchZoomEnabled()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchMode == DRAG) {
                    ...
                    if (mChart.getProperties().isScaleXEnabled()
                        || mChart.getProperties().isScaleYEnabled()
                        || mChart.getProperties().isPinchZoomEnabled()) {
                        performZoom(event);
                    }
                    ...
                    if (mChart.getViewPortHandler().hasNoDragOffset()) {

                        if (!mChart.getViewPortHandler().isFullyZoomedOut() && mChart.getProperties().isDragEnabled()) {
                            mTouchMode = DRAG;
                        } else {

                            mLastGesture = ChartGesture.DRAG;

                            if (mChart.getProperties().isHighlightPerDragEnabled())
                                performHighlightDrag(event);
                        }

                    } else if (mChart.getProperties().isDragEnabled()) {
                        mLastGesture = ChartGesture.DRAG;
                        mTouchMode = DRAG;
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                ...
                velocityTracker.computeCurrentVelocity(1000, resources.getMaximumFlingVelocity());
                final float velocityY = velocityTracker.getYVelocity(pointerId);
                final float velocityX = velocityTracker.getXVelocity(pointerId);

                if (Math.abs(velocityX) > resources.getMinimumFlingVelocity() ||
                        Math.abs(velocityY) > resources.getMinimumFlingVelocity()) {

                    if (mTouchMode == DRAG && mChart.getProperties().isDragDecelerationEnabled()) {
                        ...
                        ViewCompat.postInvalidateOnAnimation(mChart); // This causes computeScroll to fire, recommended for this by Google
                    }
                }
            ...
            case MotionEvent.ACTION_POINTER_UP:
                resources.velocityTrackerPointerUpCleanUpIfNecessary(event, mVelocityTracker);
    ...
    private void performDrag(MotionEvent event) {
        ...
        /*Log.d("TOUCH", "===========================================================");
        Log.d("TOUCH", "performDrag " + event.getX() + " " + event.getY());
        Log.d("TOUCH", "performDrag " + mTouchStartPoint.x + " " + mTouchStartPoint.y);
        Log.d("TOUCH", "performDrag " + dX + " " + dY);
        Log.d("TOUCH", "===========================================================");*/
    ...
    public boolean onDoubleTap(MotionEvent e) {
        ...
        if (mChart.getProperties().isDoubleTapToZoomEnabled()) {

            PointF trans = getTrans(e.getX(), e.getY());

            mChart.zoom(mChart.getProperties().isScaleXEnabled() ? 1.4f : 1f, mChart.getProperties().isScaleYEnabled() ? 1.4f : 1f, trans.x, trans.y);

            if (mChart.getProperties().isLogEnabled())
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        if (!mChart.getProperties().isHighlightPerTapEnabled()) {
            return false;
        }
...
//    }
    ...
    public void computeScroll() {
        ...
        mDecelerationVelocity.x *= mChart.getProperties().getDragDecelerationFrictionCoef();
        mDecelerationVelocity.y *= mChart.getProperties().getDragDecelerationFrictionCoef();
